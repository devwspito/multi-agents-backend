...
/**
 * GET /api/sandbox/files/:taskId
 * List files in the sandbox workspace as a tree structure
 * Used by UnifiedWorkspace UI to show file explorer
 */
router.get('/files/:taskId', async (req: Request, res: Response) => {
  try {
    const { taskId } = req.params;
    const maxDepth = parseInt(req.query.maxDepth as string) || 4;

    // 1. Find sandbox for this task
    const found = sandboxService.findSandboxForTask(taskId);

    if (!found) {
      // Try to get workspace path from EventStore
      const state = eventStore.getCurrentState(taskId);
      if (!state || !state.workspaces || state.workspaces.length === 0) {
        return res.json({
          success: false,
          error: 'No sandbox or workspace found for this task',
          files: [],
        });
      }

      // Return files from local workspace path
      const workspace = state.workspaces[0];
      const workspacePath = workspace.workspacePath || workspace.fullPath;

      if (!workspacePath || !fs.existsSync(workspacePath)) {
        return res.json({
          success: false,
          error: 'Workspace path not found on disk',
          files: [],
        });
      }

      const files = buildFileTree(workspacePath, '', maxDepth, 0);
      return res.json({
        success: true,
        taskId,
        workspacePath,
    ...
    // 2. Get files from sandbox via ls command
    // Use sandbox exec to list files
    const result = await sandboxService.exec(taskId, 'find /workspace -maxdepth 4 -type f -o -type d | head -500', {
      timeout: 10000,
    });
    ...
    const files = buildFileTreeFromPaths(paths, '/workspace');
    ...
    console.error('[Sandbox API] Error listing files:', error);
    return res.status(500).json({
      success: false,
...
/**
 * Build a file tree from local filesystem
 */
function buildFileTree(basePath: string, relativePath: string, maxDepth: number, currentDepth: number): any[] {
    ...
    for (const entry of filtered) {
      ...
      if (entry.isDirectory()) {
        result.push({
          ...
          path: entryPath,
          type: 'directory',
          children: buildFileTree(basePath, entryPath, maxDepth, currentDepth + 1),
        });
      } else {
        result.push({
          name: entry.name,
...
/**
 * Build a file tree from a list of paths (from sandbox exec)
 */
function buildFileTreeFromPaths(paths: string[], basePath: string): any[] {
  ...
  function convertToArray(obj: { [key: string]: any }): any[] {
    return Object.values(obj)
      .map((item: any) => ({
        name: item.name,
        path: item.path,
        type: item.type,
        children: item.type === 'directory' ? convertToArray(item.children) : undefined,
      }))
      .sort((a, b) => {
        if (a.type === 'directory' && b.type !== 'directory') return -1;
        if (a.type !== 'directory' && b.type === 'directory') return 1;
