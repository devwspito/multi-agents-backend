...
- **NO EMPTY ARRAYS**: Every repo must have at least 1 file path`;
      ...
      // 🔥 IMAGES: Convert task attachments to SDK format (Firebase Storage or legacy local)
        ...
        for (const attachmentPath of task.attachments) {
          try {
            ...
            if (attachmentPath.startsWith('uploads/') && storageService.isAvailable()) {
              ...
              NotificationService.emitConsoleLog(taskId, 'info', `  📥 Downloaded from Firebase: ${fileName}`);
            } else {
              ...
              console.log(`  🔍 Resolving local path: ${attachmentPath} -> ${localPath}`);
              ...
              if (fs.existsSync(localPath)) {
                ...
                NotificationService.emitConsoleLog(taskId, 'warn', `  ⚠️ Image file not found: ${path.basename(attachmentPath)}`);
                continue;
              }
            }
            ...
            if (imageBuffer) {
              ...
              NotificationService.emitConsoleLog(taskId, 'info', `  ✅ Image attached: ${fileName} (${fileSizeKB} KB, ${mimeType})`);
            }
          } catch (error: any) {
            const fileName = path.basename(attachmentPath);
            console.error(`  ❌ Failed to process image ${fileName}:`, error.message);
            NotificationService.emitConsoleLog(taskId, 'error', `  ❌ Failed to process image ${fileName}: ${error.message}`);
          }
        }
      }
